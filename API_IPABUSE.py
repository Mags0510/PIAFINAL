...
def cidr (z):
    ...
    if z == "":
        if os_name == "Windows":
            ...
            comando = "ifconfig | grep 'netmask' | awk '{print $4}'"
            lineabs1 = subprocess.run(comando, shell=True, capture_output=True, text=True, executable='/bin/bash')

            CDIR = lineabs1.stdout.strip() + "/24"
